...
export async function draftPostmortemDoc(params: {
  ...
  const aiCommitsInWindow: Array<{ sha: string; author: string; summary: string; tool: string | null }> = [];
  const seenAiShas = new Set<string>();
  ...
  // Also check deploys in the window
  const deploys = store.getDeployments(20, undefined, windowStart);
  ...
  // ── PR intent context for AI commits ─────────────────────────────────────
  const prIntentLines: string[] = [];
  for (const ac of aiCommitsInWindow) {
    ...
    if (ctx?.prTitle) {
      ...
      prIntentLines.push(
        ...
        `_Author: @${ctx.author ?? ac.author} · Approved by: ${approvers}${issueRefs ? ` · Refs: ${issueRefs}` : ''}_`,
  ...
  const aiCommitsSection = aiCommitsInWindow.length > 0
    ? [
      ...
      '',
      ...aiCommitsInWindow.map((c) =>
        `- \`${c.sha.slice(0, 7)}\` **${c.author}**${c.tool ? ` (${c.tool})` : ''} — ${c.summary.slice(0, 120)}`,
      ),
      '',
