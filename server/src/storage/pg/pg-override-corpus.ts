 ...
 *
 * Business logic (compactCorpus, getOverrideSummary, compileOverrideFromSlackThread)
 * is ported from override-corpus.ts and runs in-process against data fetched
 * from Postgres — same algorithm, different data source.
 */
...
export class PgOverrideCorpus implements IOverrideCorpus {
  ...
  // ── In-process compaction logic (ported from override-corpus.ts) ─────────────
  ...
  async compileOverrideFromSlackThread(
    ...
    // Extract commands inside backticks
    ...
    } else if (lower.includes('cost') || lower.includes('budget') || lower.includes('scale') || lower.includes('expensive')) {
      reason = 'cost-constraint';
      note = 'Override reason mapping: cost constraints discussed';
    } else if (lower.includes('cab') || lower.includes('freeze') || lower.includes('compliance') || lower.includes('security')) {
    ...
    } else if (lower.includes('wrong diagnosis') || lower.includes('misidentified') || lower.includes('incorrect root')) {
      reason = 'wrong-diagnosis';
      note = 'Override reason mapping: wrong root-cause diagnosis discussed';
    } else if (lower.includes('wrong fix') || lower.includes('bad command') || lower.includes('incorrect fix')) {
      reason = 'wrong-fix';
      note = 'Override reason mapping: wrong remediation command discussed';
    }

    let incidentTag = 'infra_db_connection_pool';
    if (lower.includes('oom') || lower.includes('memory') || lower.includes('limit')) {
      incidentTag = 'infra_oom_kill';
    } else if (lower.includes('rate') || lower.includes('throttl')) {
      incidentTag = 'infra_rate_limit_cascade';
    } else if (lower.includes('cert') || lower.includes('tls') || lower.includes('expiry') || lower.includes('ssl')) {
      incidentTag = 'infra_certificate_expiry';
    } else if (lower.includes('disk') || lower.includes('space') || lower.includes('full')) {
      incidentTag = 'infra_disk_pressure';
    } else if (lower.includes('slow') || lower.includes('query') || lower.includes('latency')) {
...
// ── Pure compaction helpers (ported from override-corpus.ts) ──────────────────
