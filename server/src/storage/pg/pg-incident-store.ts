 ...
 *
 * Row mapper converts snake_case DB columns and TIMESTAMPTZ values to the
 * camelCase Incident type (timestamps as epoch milliseconds).
 */
...
export class PgIncidentStore implements IIncidentStore {
  ...
  async markContextViewed(pid: string, tenantId?: string): Promise<void> {
    ...
    await sql`
      ...
      WHERE tenant_id = ${tid}
        AND pid = ${pid}
        AND context_brief_viewed_at IS NULL
    `;
  }
  ...
  async coOccurringServices(
    ...
    const sql = getSql();
    const tid = tenantId ?? DEFAULT_TENANT;
    // Convert ms to an interval string for Postgres
