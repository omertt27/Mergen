...
class AgentMemoryStore {
   ...
   */
  store(agentId: string, key: string, value: string, ttlMs = 0): AgentMemoryEntry {
    ...
    this.db.run(
      `INSERT OR REPLACE INTO agent_memory (id, agent_id, key, value, stored_at, ttl_ms)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, agentId, key, value, now, ttlMs],
    );
    this.flush();

    return { id, agentId, key, value, storedAt: now, ttlMs };
  }

  /**
   * Retrieve memory entries, filtered by agentId and/or key.
   * Expired entries (ttlMs > 0 and storedAt + ttlMs < now) are excluded.
   */
  recall(agentId?: string, key?: string, limit = 20): AgentMemoryEntry[] {
    if (!this.db) return [];
